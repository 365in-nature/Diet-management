      ...
      <svg className="chart" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
        ...
        {pts.map((p, i) => (
          <g key={i}>
            ...
            {showDiff && p.diffPct !== null && i === pts.length - 1 ? (
              <text x={p.x} y={p.y - 8} textAnchor="middle" fontSize="10" fontWeight="600" fill={color}>
                {p.d[valueKey]} <tspan fill={parseFloat(p.diffPct) < 0 ? "#2d6a4f" : "#e07a5f"} fontSize="9">({parseFloat(p.diffPct) > 0 ? "+" : ""}{p.diffPct}%)</tspan>
              </text>
            ) : (
              <text x={p.x} y={p.y - 8} textAnchor="middle" fontSize="10" fontWeight="600" fill={color}>
                {p.d[valueKey]}
      ...
      const makeSVG = (data, valueKey, color, label, unit, showPctChange = false) => {
        ...
        return `<svg width="100%" viewBox="0 0 ${w} ${h}" style="display:block;max-width:100%">
          ...
          ${pts.map(p => {
            ...
            ${pctChange ? `
            <text x="${p.x}" y="${p.y - 8}" text-anchor="middle" font-size="10" font-weight="600" fill="${color}">${p.val} <tspan fill="${pctColor}" font-size="9">(${parseFloat(pctChange) > 0 ? "+" : ""}${pctChange}%)</tspan></text>` : `
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media print {
    @page { margin: 20mm 14mm 16mm 14mm; size: A4; }
    body { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    .no-break { page-break-inside: avoid; }
    .pdf-logo { display: block !important; }
    .pdf-pagenum { display: block !important; }
  }
  /* 로고 - 상단 중앙 고정 */
  .pdf-logo {
    display: none;
